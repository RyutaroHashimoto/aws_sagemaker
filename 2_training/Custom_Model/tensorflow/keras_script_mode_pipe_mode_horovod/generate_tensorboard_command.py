...
for job in running_jobs['TrainingJobSummaries']:
    ...
    if tensorboard_job:
        ...
        if logdir is None:
            logdir = '{}'.format(tensorboard_dir)
        else:
            logdir = '{},{}'.format(logdir, tensorboard_dir)
            print(logdir)

if logdir:
    print('AWS_REGION={} tensorboard --logdir {} --bind_all'.format(boto3.session.Session().region_name, logdir))
else:
